...
mod tests {
    ...
    fn test_hook_command_preview_bundled_shape_with_quoted_args() {
        let preview = ClaudeAdapter.hook_command_preview(
        ...
        assert_eq!(
            preview,
            "${CLAUDE_PLUGIN_ROOT}/hooks/scripts/_wrappers/pre_tool_use.sh ${CLAUDE_PLUGIN_ROOT}/hooks/scripts/audit.sh audit-bash '--strict' 'two words'"
        );
        assert!(
            preview.contains("${CLAUDE_PLUGIN_ROOT}"),
