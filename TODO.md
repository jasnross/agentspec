...
9. Provider-payload translation shim for hook scripts
   ...
   - Adapters own the provider→canonical mapping; `compile.rs`/`sync.rs` stay provider-agnostic.
   ...
   - Out of scope for the hooks-pipeline branch; revisit as part of post-1.0 cross-provider ergonomics.
10. Allow specifying multiple events for a single hook in `hooks.toml`
...
11. Verbose parity for `agentspec remove`
...
12. Support sharing spec content (helper scripts, fragments) across multiple agentspec spec directories
...
13. Hoist sync-time validations to config-load time
...
14. Add a project rule about hoisting validations to config-validation time
...
15. Consider splitting `ConfigPatch` into `ForwardPatch` + `ReversePatch` traits
...
16. Consider how to support provider-specific specs in general (hooks and beyond)
...
17. Support `email` in `plugin-author` for plugin-mode manifest emission
...
18. Auto-install `jq` for plugin-mode hooks via the plugin persistent-data directory
