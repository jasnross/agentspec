...
11. Verify Claude's `effort:` rendering against a running provider
    ...
    - **The oracle is the expensive one.** A recording proxy via `ANTHROPIC_BASE_URL`, asserting on `output_config.effort` in the outbound request body. It is the only oracle of the three that reaches the end of the chain `preset config → emitted bytes → provider parses file → provider's resolved config → outbound model request`, and the only one needing a live session plus a substrate to build
    - **Open: which capture substrate.** aimock, mitmproxy, and a custom recorder are all unevaluated. Whichever is chosen has to work from a probe package's `probe.sh`, since a manifest is only authored once its assertion is validated to discriminate
    - **Open: whether a cheaper Claude oracle exists.** The transcript-JSONL candidate was tested and eliminated — `output_config` appears as a structural field in none of 79 local transcripts. `/status`, `--debug`/`--verbose`, and OTEL export remain untested. One probe package settling this is worth doing before any proxy gets built
