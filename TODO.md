...
20. _Done — see `thoughts/plans/2026-05-08-delete-empty-host-config-on-remove.md`._ Each adapter's remove post-write hook now deletes the host config file (`.claude/settings.json`, `.cursor/hooks.json`, `.opencode/opencode.json`) when (a) tidy actually removed at least one agentspec-owned entry, and (b) the residual file is empty per a per-adapter predicate (Claude/OpenCode: `{}`; Cursor: `{}` or single `version` key, any value). After a delete, the host file's parent directory is best-effort `rmdir`'d, so `.claude/`, `.cursor/`, and `.config/opencode/` are cleaned up too when only agentspec content was present. User-authored top-level keys (e.g. `permissions`, custom Cursor keys, OpenCode `model`) keep the file alive.
...
22. _Done — see `thoughts/plans/2026-05-08-consolidate-inline-sync-config-writes.md`._ The `write_sync_config` helper's element type was extended from `(&str, &str)` to a `SyncEntry<'a>` struct with optional `prefix`/`content_prefix`/`dir` builder methods; existing tuple callers were mechanically updated and inline `[sync.<provider>]` writes were migrated. Two sites remain inline by design (see `// Inline write retained intentionally` comments at their call sites): one writes intentionally-malformed TOML to provoke a parser error (`test_sync_invalid_base_sync_config_surfaces_parse_error`); the other writes a `[spec] ignore = [...]` block that is the configuration under test (`test_sync_dry_run_lists_ignored`). Slot kept to preserve numbering of subsequent items.
