...
11. ~~Hoist sync-time validations to config-load time~~ — RESOLVED: `validate_for_provider` now checks both `plugin-name` and `dir` constraints; `agentspec validate` calls `validate_sync_config` to surface config errors early; use-site check in `resolve_dest_dir` downgraded to `debug_assert!`
...
12. ~~Add a project rule about hoisting validations to config-validation time~~ — RESOLVED: `.claude/rules/validation-locality.md` added
