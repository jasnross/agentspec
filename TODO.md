...
4. Consider merging `bash` and `powershell` under a single `shell` canonical tool
...
12. Adapter API consolidation (next phase after the `ProviderAdapter` trait extraction)
    ...
    - Pairs naturally with #9's payload translation shim
...
13. Support sharing spec content (helper scripts, fragments) across multiple agentspec spec directories
...
14. Hoist sync-time validations to config-load time
...
15. Add a project rule about hoisting validations to config-validation time
...
16. Consider splitting `ConfigPatch` into `ForwardPatch` + `ReversePatch` traits
