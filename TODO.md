...
9. Allow specifying multiple events for a single hook in `hooks.toml`
...
10. Verbose parity for `agentspec remove`
...
11. Support sharing spec content (helper scripts, fragments) across multiple agentspec spec directories
...
12. Hoist sync-time validations to config-load time
...
13. Add a project rule about hoisting validations to config-validation time
...
14. Consider splitting `ConfigPatch` into `ForwardPatch` + `ReversePatch` traits
...
15. Consider how to support provider-specific specs in general (hooks and beyond)
...
16. Support `email` in `plugin-author` for plugin-mode manifest emission
