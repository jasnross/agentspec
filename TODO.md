...
14. Verbose parity for `agentspec remove`
...
15. Adapter API consolidation (next phase after the `ProviderAdapter` trait extraction)
    ...
    - Generalizes / supersedes the path-method-shape concerns in #5 and the JSON-merge concerns in #11; pairs naturally with #12's payload translation shim
...
16. Support sharing spec content (helper scripts, fragments) across multiple agentspec spec directories
