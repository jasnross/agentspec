...
1. **Load** — `specs.rs` (`Specs::load`) reads `.md` files from `spec/agents/`, `spec/skills/`, `spec/rules/`, and `spec/hooks/hooks.toml` (plus a recursive walk of `spec/hooks/scripts/`), parses frontmatter via `gray_matter` (markdown specs) and `serde_path_to_error` over `toml` (hooks) into typed structs → `(Specs, LoadReport)`. Files (and subtrees) matching any pattern in `[spec].ignore` are skipped before frontmatter parsing via `WalkDir::filter_entry` pruning; the `LoadReport` records what was filtered and which patterns matched zero files so `main.rs` can surface warnings and listings. Every walk follows symlinks (`WalkDir::follow_links(true)`; the `read_dir` scans under `skills/` route entries through `resolve_dir_entry`, since `DirEntry::file_type` describes the link rather than its target), resolving them to their target content at load time and emitting that content as regular files in compiled output. A symlink target may resolve anywhere, including outside `sources_dir` — this is how one spec directory draws specs or supporting files from a pool shared with other spec directories, and agentspec takes no stance on where that pool lives. Dangling targets and symlink loops produce clear compile-time errors via `resolve_walk_entry` for the `WalkDir` passes and `resolve_dir_entry` for the `read_dir` scans, so no walk discards a failed entry with `filter_map(Result::ok)`. A resolving directory link whose target encloses the directory the link sits in is refused by `reject_ancestor_loop` — at every walk root, at every directory `resolve_walk_entry` yields (`walkdir` yields a directory before descending into it), and at every `skills/` entry that survived its ignore prune. Such a link resolves, so unlike a broken one it is still a link the load has not tried, and a pattern naming it prunes it wherever this guard is what catches it. Two cases it is not: `walkdir` runs its own loop detection while producing an entry, ahead of `filter_entry`, so a link whose target is the root of the walk it sits in — or an ancestor inside that walk — errors before any pattern is consulted. That covers every `WalkDir` pass, each skill directory included, since `load_single_skill` walks one as its own root; only the `read_dir` scan over the entries directly under `skills/` reaches every enclosing link. A skill whose only spec file is ignored is skipped by `select_spec_md` before its walk is built, so a pattern naming that file suppresses what the walk would have reported — a third route by which a pattern reaches a link, after pruning an ancestor and naming the link. The other case is a spec root that is itself such a link, checked before the root prune, so ignoring the root does not exempt it either. Left to `walkdir`, that link is reported only after its target has been read, and named by the second encounter of the link reached through itself. An entry that fails to resolve never reaches `filter_entry`, so a pattern naming the link itself does not exempt it; only a pattern pruning an ancestor directory keeps the walk from reaching it at all. Both `read_dir` scans are stricter still — each stats every entry before the ignore check that would exclude it, so an ignored skill directory or supporting file that is a broken link fails the load. A spec root that is itself a symlink fails through `spec_dir_present` / `spec_file_present`, which distinguish a root the author never created from one authored as a link that does not resolve — or resolves to the wrong kind. `sources_dir` gets stricter treatment one level up, in `SpecDirs::new` — the four roots are joins onto it, so an absent, wrong-kind, or unresolvable `sources_dir` would otherwise leave all four looking merely absent, and `sync` would treat every installed file as stale. An absent `sources_dir` is therefore an error even though each of the four roots under it is optional. Deriving the roots inside the constructor is what keeps that check from being bypassed by spelling the joins out again. `main.rs` builds `SpecDirs` per subcommand rather than once, because `remove` consults only the manifest and an unresolvable `sources_dir` must not be what blocks an uninstall. Cross-platform note: macOS/Linux are supported; Windows users on git clones with `core.symlinks = false` will see symlinks as regular files containing target paths (not handled).
