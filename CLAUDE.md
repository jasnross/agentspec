...
1. **Load** — `specs.rs` (`Specs::load`) reads `.md` files from `spec/agents/`, `spec/skills/`, `spec/rules/`, and `spec/hooks/hooks.toml` (plus a recursive walk of `spec/hooks/scripts/`), parses frontmatter via `gray_matter` (markdown specs) and `serde_path_to_error` over `toml` (hooks) into typed structs → `(Specs, LoadReport)`. An `[spec].ignore` pattern decides membership in the loaded set before anything is resolved: every point that could exclude an entry — `admit_spec_dir` at the four spec roots, `hooks.toml`, and `hooks/scripts`; `walk_spec_tree` for each entry and each failed entry of every walk; both `read_dir` scans — applies the pattern first and stats second. Excluded means excluded, so naming a dangling or looping link exempts it exactly as pruning its parent does, and no load-stage check consults the disk about an excluded path. `LoadReport::record` enforces single-recording, so a path two sites both reach is one ignored path with one hit; it records what was filtered and which patterns matched zero files so `main.rs` can surface warnings and listings. Every walk follows symlinks (`WalkDir::follow_links(true)`; the `read_dir` scans under `skills/` route entries through `resolve_dir_entry`, since `DirEntry::file_type` describes the link rather than its target), resolving them to their target content at load time and emitting that content as regular files in compiled output. A symlink target may resolve anywhere, including outside `sources_dir` — this is how one spec directory draws specs or supporting files from a pool shared with other spec directories, and agentspec takes no stance on where that pool lives. An unignored dangling target or symlink loop produces a clear compile-time error via `walk_error` for the `WalkDir` passes and `resolve_dir_entry` for the `read_dir` scans, so no walk discards a failed entry with `filter_map(Result::ok)`. `walk_spec_tree` drives `WalkDir` by hand rather than through `filter_entry`, because `filter_entry` propagates a failed entry without consulting its predicate — a pattern could otherwise never reach a broken link, or a cycle `walkdir` caught while producing the entry. A resolving directory link whose target encloses the directory the link sits in is refused by `reject_ancestor_loop` — at every admitted walk root, at every directory `walk_spec_tree` yields (`walkdir` yields a directory before descending into it), and at every `skills/` entry that survived its ignore check. Left to `walkdir`, that link is reported only after its target has been read, and named by the second encounter of the link reached through itself. A `hooks.toml` entry declaring a script that a pattern excluded fails the load with a diagnostic naming the hook, the script, and the pattern — the membership test reads `supporting_files` rather than stat'ing the script, so an excluded script can no longer produce a hook command pointing at a file agentspec never writes. A spec root that is itself a symlink fails through `spec_dir_present` / `spec_file_present`, which distinguish a root the author never created from one authored as a link that does not resolve — or resolves to the wrong kind. `sources_dir` gets stricter treatment one level up, in `SpecDirs::new` — the four roots are joins onto it, so an absent, wrong-kind, or unresolvable `sources_dir` would otherwise leave all four looking merely absent, and `sync` would treat every installed file as stale. An absent `sources_dir` is therefore an error even though each of the four roots under it is optional. Deriving the roots inside the constructor is what keeps that check from being bypassed by spelling the joins out again. `main.rs` builds `SpecDirs` per subcommand rather than once, because `remove` consults only the manifest and an unresolvable `sources_dir` must not be what blocks an uninstall. Cross-platform note: macOS/Linux are supported; Windows users on git clones with `core.symlinks = false` will see symlinks as regular files containing target paths (not handled).
